	...
	<div class="container">
		<div class="order-details">
			...
			<p>
				...
				<%=order.getOrderTotal()%> (Incl. taxes	)</p>
			<p><strong>Shipping Charges</strong> 
				<%=session.getAttribute("shipping") %>
			</p>
			<hr>
			...
			<div class="address-details">
				...
				<p>
					...
					<%=address.getLocation()%></p>
				<p>
					<strong>Address:</strong>
					<%=address.getAddress()%></p>
			</div>
		</div>
		<button id="razorpay-btn">Pay Now</button>

		<script>
        var orderId = "<%=session.getAttribute("orderId")%>";
        var amount = "<%=session.getAttribute("orderAmount")%>";
			...
			var options = {
				...
				"handler" : function(response) {
					...
					// Redirect or perform any other action after successful payment
					 window.location.href = "http://localhost:9090/flipkart/Confirmation.jsp?payment_id=" + response.razorpay_payment_id;
				},
				"prefill" : {
